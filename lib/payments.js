...
async function confirmPayment(payment) {
  ...
  if (user) {
    const { renewalDate, extendPremium, REFERRAL_THRESHOLD, REFERRAL_BONUS_DAYS } = require('./subscription');
    user.plan = 'active';
    user.planEnds = renewalDate(user.planEnds);
    await users.save(user);
    if (user.refBy) {
      const referrer = users.findByRefCode(user.refBy);
      if (referrer && referrer.id !== user.id) {
        referrer.confirmedRefs = (referrer.confirmedRefs || 0) + 1;
        if (referrer.confirmedRefs % REFERRAL_THRESHOLD === 0) {
          extendPremium(referrer, REFERRAL_BONUS_DAYS);
        }
        await users.save(referrer);
      }
    }
  }
}
