...
async function write(key, value) {
  ...
  if (upstash) {
    const res = await fetch(
      upstash.url + '/set/' + encodeURIComponent(String(key)) + '/' + encodeURIComponent(str),
      { method: 'POST', headers: { Authorization: 'Bearer ' + upstash.token }, signal: AbortSignal.timeout(8000) }
    );
    if (!res.ok) console.error('[db] Upstash write failed for "' + key + '": HTTP ' + res.status + ' body=' + (await res.text().catch(() => '')).slice(0, 200));
    return;
  }
...
module.exports = { init, flush, read, users, trades, payments, config, storageMode: STORAGE_MODE, hasUpstash: !!upstash };