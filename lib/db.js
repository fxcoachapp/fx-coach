...
async function read(key) {
  if (upstash) {
    try {
      const result = await upstash.get(String(key));
...
async function write(key, value) {
  ...
  if (upstash) {
    try {
      await upstash.set(String(key), str);
    } catch (e) {
...
module.exports = { init, flush, read, redisOf: () => upstash, users, trades, payments, config, storageMode: STORAGE_MODE, hasUpstash: !!upstash };