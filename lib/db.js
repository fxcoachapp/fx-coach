...
function timed(promise, ms, label) {
  return new Promise((resolve, reject) => {
    const t = setTimeout(() => {
      reject(new Error((label || 'upstash') + ' timed out after ' + ms + 'ms'));
    }, ms);
    promise.then(
...
async function read(key) {
  if (upstash) {
    try {
      const result = await timed(upstash.get(String(key)), 8000, 'upstash get');
      if (result === null || result === undefined) return defaults(key);
...
async function write(key, value) {
  ...
  if (upstash) {
    try {
      await timed(upstash.set(String(key), str), 8000, 'upstash set');
    } catch (e) {
      console.error('[db] Upstash write failed for "' + key + '": ' + String(e && e.message || e).slice(0, 200));
    }
    return;
  }
...
module.exports = { init, flush, read, timed, redisOf: () => upstash, users, trades, payments, config, storageMode: STORAGE_MODE, hasUpstash: !!upstash };